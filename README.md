...
"저평가 매물"(워치리스트 중앙값 대비 -15% 이하)을 GitHub Pages 대시보드에 표시.
...
- `scripts/collect.js` — 수집 + 저장 + 저평가 판정 (10분마다 실행됨)
...
| `DEVIATION_ALERT_PCT` | 0.15 | 중앙값 대비 몇 % 낮으면 대시보드에 표시할지 |
...
- 대시보드는 10분 주기 수집 직후에만 갱신되므로 실시간 알림은 아님(Slack 푸시
  대신 사이트를 직접 확인해야 함).
