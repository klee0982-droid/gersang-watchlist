...
- `scripts/collect.js` — 수집 + 저장 + 저평가 판정 (기본 15분마다, 즉시 갱신은 수동 버튼)
...
## 수집 주기: 기본(15분 자동) + 즉시 새로고침(수동)

**GitHub Actions의 schedule 트리거는 "베스트 에포트"라 부하 상황에서 조용히
누락/지연될 수 있음** (공식 문서에 명시된 한계). 실측 결과(2026-08-09) `*/15`로
설정해도 실제로는 시간당 1회 정도로 밀리는 걸 확인함 — 반면 수동 실행
(`workflow_dispatch`)은 지금까지 100% 즉시 성공했음. 그래서 아래처럼 이원화함:

- **기본 주기(15분 설정)**: 항상 자동으로 시도됨. 다만 위 이유로 실제로는
  더 늦게(시간 단위로) 돌 수 있음 — DB에 꾸준히 데이터가 쌓이는 용도로는 충분.
- **즉시 새로고침**: 대시보드 상단의 **"🔄 지금 새로고침"** 버튼 클릭 →
  GitHub Actions "Run workflow" 페이지로 이동 → 초록색 **Run workflow** 버튼
  클릭. 게임 할 때 등 최신 시세가 바로 필요하면 이 버튼을 쓰면 됨
  (외부 서비스나 토큰 노출 없이, GitHub 자체 기능만 사용).
...
- 대시보드는 수집 직후에만 갱신되므로 실시간 알림은 아님. 자동 주기가
  GitHub 스케줄러 특성상 불규칙하므로, 최신 정보가 필요할 땐 "🔄 지금
  새로고침" 버튼으로 직접 트리거하는 걸 기본 워크플로우로 삼을 것
  (Slack 푸시처럼 알아서 오는 게 아님).
- 거타 이용약관/robots.txt에 자동 수집을 명시적으로 금지하는 조항은
  발견되지 않았으나, 완전한 법적 검토는 아니므로 과도하게 잦은 요청은 피할 것.
  기본 주기만이면 하루 약 960요청(15분 × 10페이지) 예상이나, 실제로는 스케줄
  지연으로 더 적게 나갈 가능성이 큼. 만약 geota가 접근을 차단하는 정황이
  보이면 기본 주기(`PAGES_PER_SERVER`, cron)를 더 늘릴 것.
