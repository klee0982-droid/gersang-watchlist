...
function renderInsightRow(insight, rank) {
  const pct = `${(insight.avgDeviationPct * 100).toFixed(1)}%`;
  return `
    <tr>
      <td class="num muted">${rank}</td>
      <td>${escapeHtml(insight.itemName)}</td>
      <td>${insight.groupLabel ? escapeHtml(insight.groupLabel) : '<span class="muted">-</span>'}</td>
      <td class="num">${insight.alertCount}</td>
      <td class="num neg">${pct}</td>
      <td class="num">${insight.sampleCount ?? '<span class="muted">-</span>'}</td>
      <td class="num">${insight.medianPrice != null ? formatWon(insight.medianPrice) : '<span class="muted">-</span>'}</td>
    </tr>`;
}
...
export function renderSiteHtml({ alerts, watchlist, priceTrends, insights, insightWindowDays, generatedAt, alertWindowHours }) {
  const alertRows = alerts.length
    ? alerts.map(renderAlertRow).join('')
    : `<tr><td colspan="7" class="empty">최근 ${alertWindowHours}시간 내 저평가 매물이 없습니다.</td></tr>`;
  ...
  const insightRows = insights && insights.length
    ? insights.map((ins, i) => renderInsightRow(ins, i + 1)).join('')
    : `<tr><td colspan="7" class="empty">아직 데이터가 부족해 인사이트를 계산할 수 없습니다.</td></tr>`;
...
<main>
  ...
  <h2>🎯 지켜볼 가치가 있는 아이템 (최근 ${insightWindowDays}일)</h2>
  <p class="subtitle" style="margin-top:-0.5rem;">알림 횟수 × 평균 할인율 기준 상위 ${insights?.length ?? 0}개 — 자주, 깊게 저평가로 뜨는 아이템일수록 위에 있음. 표본 수가 적으면 중앙값 자체가 부정확할 수 있으니 참고.</p>
  <div class="card">
    <table>
      <thead>
        <tr>
          <th>#</th><th>아이템</th><th>그룹</th><th>알림 횟수</th><th>평균 할인율</th><th>표본 수</th><th>중앙값</th>
        </tr>
      </thead>
  ...
  <h2>🔻 최근 저평가 매물 (${alertWindowHours}시간 이내)</h2>
  <div class="card">
    <table>
      <thead>
        <tr>
